...
//ajax call referring to ticketmaster
    $.ajax({
      ...
      for (var i = 0; i < response._embedded.events.length; i++) {
        $("#events").append("<h5> Event: " + response._embedded.events[i].name + "</h5>" + "<h6>" + response._embedded.events[i].url + "</h6>");
      }
    })
  }
    ...
    $.ajax({
      ...
      $.ajax({
        ...
        for (var i = 0; i < response.collections.length; i++) {
          $("#food").append("<h5> Food Description: " + response.collections[i].collection.description + "</h5>" + "<h6>" + response.collections[i].collection.url + "</h6>");
        }
      })
    })
  }
